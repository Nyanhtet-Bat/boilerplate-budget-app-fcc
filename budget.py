class Category:
  ...
  def transfer(self, amount, budget_category):
    if self.check_funds(amount)==True:
      self.ledger.append({'amount': -(amount),'description':
      f'Transfer to {budget_category.name}'})
...
def create_spend_chart(categories):
  ...
  for i in categories:
    ...
    for j in i.ledger:
      if j['amount'] < 0 :
        s += abs(j['amount'])
    spend_dict[i.name] = round(s,2)
  total = sum(spend_dict.values())
  precent_dict = {}
  for k in spend_dict.keys():
    precent_dict[k] = int(round(spend_dict[k]/total,2)*100)
  output = 'Percentage spent by category\n'
  for i in range(100, -10, -10):
    output += f'{i}'.rjust(3) + '| '
    for percent in precent_dict.values():
      if percent >= i:
        output += 'o  '
      else:
        output += '   '
    output += '\n'
  output += ' '*4 + '-'*(len(precent_dict.values())*3+1)
  output += '\n     '
  dict_key_list = list(precent_dict.keys())
